...
func RegisterHttpHandler(router *mux.Router, endpoints Endpoints, tracer stdopentracing.Tracer, logger log.Logger) {
	...
	router.Methods("GET").Path("/hello-world/v1/echos/{name}").Handler(
		httptransport.NewServer(
			endpoints.GetEchoEndpoint,
			DecodeHTTPGetEchoZeroRequest,
			EncodeHTTPGenericResponse,
			addTracerOption("get_echo")...,
		//append(serverOptions, httptransport.ServerBefore(opentracing.HTTPToContext(tracer, "get_echo", logger)))...,
		))
}
...
// DecodeHTTPGetEchoZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded get_echo request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPGetEchoZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	var req pb.GetEchoRequest
