...
import (
	...
	// This Service
	"git.company.com/examples/hello-world/service-go/internal/hello-world-server"
)
...
func main() {
	...
	err := config.ScanFrom(&conf, "helloWorldServer", "server")
	if err != nil {
	...
	if len(GitHash) == 0 {
		logs.Infow("starting HelloWorldServer. you can run `go generate` before `go build`, then there will be git info here.")
	} else {
		logs.Infow("starting HelloWorldServer", "gitHash", GitHash, "gitBranch", GitBranch, "buildTime", BuildTime)
	}
	server.Run(conf)
}
