...
func RegisterHttpHandler(router *mux.Router, endpoints Endpoints, tracer stdopentracing.Tracer, logger log.Logger) {
	...
	router.Methods("GET").Path("/user/v1/users/{id}").Handler(
		httptransport.NewServer(
			endpoints.GetUserinfoEndpoint,
			DecodeHTTPGetUserinfoZeroRequest,
			EncodeHTTPGenericResponse,
			addTracerOption("get_userinfo")...,
		//append(serverOptions, httptransport.ServerBefore(opentracing.HTTPToContext(tracer, "get_userinfo", logger)))...,
		))
}
...
// DecodeHTTPGetUserinfoZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded get_userinfo request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPGetUserinfoZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	...
	err = mjhttp.UnmarshalQueryParam(queryParams, &req.Name, "name")
