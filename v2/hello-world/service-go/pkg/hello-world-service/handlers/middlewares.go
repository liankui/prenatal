...
import (
	...
	"git.company.com/examples/hello-world/service-go/pkg/hello-world-service/svc"
...
func WrapEndpoints(in svc.Endpoints, options map[string]interface{}) svc.Endpoints {
	...
	{ // get_echo
		if tracer != nil {
			in.GetEchoEndpoint = opentracing.TraceServer(tracer, "get_echo")(in.GetEchoEndpoint)
		}
		if count != nil && latency != nil {
			in.GetEchoEndpoint = middleware.Instrumenting(latency.With("method", "get_echo"), count.With("method", "get_echo"))(in.GetEchoEndpoint)
		}
		//if validator != nil {
		//	in.GetEchoEndpoint = validator.Validate()(in.GetEchoEndpoint)
		//}
	}
...
func WrapService(in pb.HelloWorldServer, options map[string]interface{}) pb.HelloWorldServer {
	return in
}
