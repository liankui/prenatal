...
import (
	...
	"git.company.com/examples/hello-world/service-go/pkg/hello-world-service/handlers"
...
func NewEndpoints(options map[string]interface{}) svc.Endpoints {
	// Business domain.
	var service pb.HelloWorldServer
	{
	...
	endpoints := svc.Endpoints{
		GetEchoEndpoint: getEchoEndpoint,
	}
...
func RegisterService(cfg nserver.Config, r *mux.Router, s *grpc.Server) svc.Endpoints {
	const FullServiceName = "hello_world.HelloWorld"
	...
	pb.RegisterHelloWorldServer(s, svc.MakeGRPCServer(endpoints, tracer, logger))
...
func Run(cfg nserver.Config) {
	...
	// Run!
	logs.Info("hello_world.HelloWorldServer", " started.")
	logs.Info("hello_world.HelloWorldServer", <-errc)

	logs.Info("hello_world.HelloWorldServer", " closed.")
}
