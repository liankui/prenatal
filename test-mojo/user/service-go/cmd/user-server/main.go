...
import (
	...
	// This Service
	"github.com/liankui/prenatal-server/service-go/internal/user-server"
)
...
func main() {
	...
	err := config.ScanFrom(&conf, "userServer", "server")
	if err != nil {
	...
	if len(GitHash) == 0 {
		logs.Infow("starting UserServer. you can run `go generate` before `go build`, then there will be git info here.")
	} else {
		logs.Infow("starting UserServer", "gitHash", GitHash, "gitBranch", GitBranch, "buildTime", BuildTime)
	}
	server.Run(conf)
}
