...
import (
	...
	"github.com/liankui/prenatal-server/service-go/pkg/user-service/handlers"
...
func RegisterService(cfg nserver.Config, r *mux.Router, s *grpc.Server) svc.Endpoints {
	const FullServiceName = "user.User"
	...
	pb.RegisterUserServer(s, svc.MakeGRPCServer(endpoints, tracer, logger))
...
func Run(cfg nserver.Config) {
	...
	// Run!
	logs.Info("user.UserServer", " started.")
	logs.Info("user.UserServer", <-errc)

	logs.Info("user.UserServer", " closed.")
}
